...
<body>
	...
	<%@ include file="/admin/html/include/container.jsp" %>
		<section id="contents">
			...
			<form name="frm" action="/admin/file/write.action" method="post" enctype="multipart/form-data">
				<input type="hidden" name="id" value="<c:out value="${user.id }" />" />
				<input type="hidden" name="name" value="<c:out value="${user.name }" />" />
				
				<table class="table_global">
					<caption>다운로드 게시판에 등록할 게시물의 카테고리, 제목, 내용, 첨부파일을 입력할 수 있습니다.</caption>
					...
					<tbody>
						<tr>
							...
							<td><span class="inputfield_outer"><input type="text" name="category" class="inputfield_global" value="다운로드" readonly /></span></td>
						</tr>
						<tr>
							<th scope="row">제목</th>
	...
	<script>
		var count = 0;
		console.log("[file] [write.jsp] count: ", count);
