	...
	<%@ include file="/admin/html/include/container.jsp" %>
		<section id="contents">
					...
					<c:choose>
						<c:when test="${not empty boardList }">
							<c:forEach var="boardList" items="${boardList }">
								<tr>
									<td><c:out value="${boardList.number }" /></td>
									<td><c:out value="${boardList.category }" /></td>
									<td class="subject">
										<c:choose>
											<c:when test="${boardList.replyDepth != 0}">
												<c:forEach var="i" begin="0" end="${boardList.replyDepth - 1}" varStatus="status">
												*
												</c:forEach>
												답변: 
											</c:when>
										...
										<a href="/admin/board/view.do?number=<c:out value="${boardList.number }" />"><c:out value="${boardList.subject }" /></a>
									</td>
									<td><c:out value="${boardList.name }" /></td>
									<td><fmt:formatDate value="${boardList.regdate }" type="date" /></td>
									<td><c:out value="${boardList.count }" /></td>
								</tr>
							</c:forEach>
						</c:when>
						<c:otherwise>
			...
			<c:if test="${not empty boardList }">
				<div class="paging">
					<c:choose>
						...
						<c:otherwise>
							<a href="/admin/board/list.do?page=<c:out value="${currentPage - 1 }" />" class="previous">이전</a>
						</c:otherwise>
					</c:choose>
						...
						<c:forEach var="i" begin="${startPage }" end="${endPage }" step="1" varStatus="status">
							<c:choose>
								<c:when test="${i == currentPage }">
									<a href="/admin/board/list.do?page=<c:out value="${i }" />" class="current"><c:out value="${i }" /><span class="invisible">현재 페이지</span></a>
								</c:when>
								
								<c:otherwise>
									<a href="/admin/board/list.do?page=<c:out value="${i }" />"><c:out value="${i }" /></a>
								</c:otherwise>
							</c:choose>
						</c:forEach>
					</div><!-- // inner_paging -->
					...
					<c:choose>
						...
						<c:otherwise>
							<a href="/admin/board/list.do?page=<c:out value="${currentPage + 1 }" />" class="next">다음</a>
						</c:otherwise>
					</c:choose>
				</div><!-- // paging -->
			</c:if>
			...
			<div class="board_search">
				<form action="/admin/board/list.do" method="get">
			...
			<div class="board_gravity">
				<div class="inner_gravity">
					<a href="/admin/board/write.do" class="button_global">등록</a>
				</div><!-- // inner_gravity -->
			</div><!-- // board_gravity -->
		</section><!-- // contents -->
