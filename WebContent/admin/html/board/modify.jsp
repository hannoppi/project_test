...
<body>
	...
	<%@ include file="/admin/html/include/container.jsp" %>
		<section id="contents">
			...
			<form name="frm" action="/admin/board/modify.action" method="post">
				<input type="hidden" name="id" value="<c:out value="${user.id }" />" />
				<input type="hidden" name="name" value="<c:out value="${user.name }" />" />
				<input type="hidden" name="number" value="<c:out value="${BoardModifyView.number }" />" />
				
				<table class="table_global">
					<caption>일반 게시판에 등록된 게시물의 카테고리, 제목, 내용을 수정할 수 있습니다.</caption>
					...
					<tbody>
						<tr>
							...
							<td>
								<span class="inputfield_outer"><input type="text" name="category" class="inputfield_global" value="<c:out value="${BoardModifyView.category }" />" readonly /></span>
							</td>
						</tr>
						<tr>
							<th scope="row">제목</th>
							<td>
								<span class="inputfield_outer"><input type="text" name="subject" class="inputfield_global" value="<c:out value="${BoardModifyView.subject }" />" /></span>
							</td>
						</tr>
						<tr class="row_local">
							<th scope="row">내용</th>
							<td>
								<span class="textfield_outer"><textarea name="content" id="content" class="textfield_global"><c:out value="${BoardModifyView.content }" /></textarea></span>
							</td>
						</tr>
					</tbody>
				</table><!-- // table_global -->
