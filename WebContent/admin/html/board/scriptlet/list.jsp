		...
		<section id="contents">
						...
						<c:when test="${not empty boardList }">
							<c:forEach var="boardList" items="${boardList }">
								<tr>
									...
									<td class="subject"><a href="/admin/board/view.do?number=${boardList.number }">${boardList.subject }</a></td>
			...
			<c:if test="${not empty boardList }">
				<div class="paging">
					...
					<% } else { %>
					<a href="/admin/board/list.do?page=<%= currentPage - 1 %>" class="previous">이전</a>
					<% } %>
					
					<div class="inner_paging">
						<%for(int i = startPage; i <= endPage; i++) {
						if(i == currentPage) { %>
						<a href="/admin/board/list.do?page=<%=i %>" class="current"><%=i %><span class="invisible">현재 페이지</span></a>
						<% } else { %>
						<a href="/admin/board/list.do?page=<%=i %>"><%=i %></a>
						<% }
						} %>
					</div><!-- // inner_paging -->
					...
					<% } else { %>
					<a href="/admin/board/list.do?page=<%=currentPage + 1 %>" class="next">다음</a>
					<% } %>
				</div><!-- // paging -->
			</c:if>
			...
			<div class="board_search">
				<form action="/admin/board/list.do" method="get">
					<div class="design_select">
						...
						<select name="option" id="selectCategory">
			...
			<div class="board_utility">
				<div class="inner_utility">
					<a href="/admin/board/write.do" class="button_global button_medium button_black">등록</a>
				</div><!-- // inner_utility -->
			</div><!-- // board_utility -->
		</section><!-- // contents -->
