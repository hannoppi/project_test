		...
		try{
			if( !oNavigator.ie ){
				if(!!oNavigator.safari && oNavigator.version <= 5){
					...
					bSupportDragAndDropAPI = true; // false: 크롬에서도 싱글 파일업로드
				}
			} else {
				bSupportDragAndDropAPI = false;
			}
	...
	function onAjaxError (){
 	...
 	function callFileUploader (){
 		console.log("location.href.replace(/\/[^\/]*$/, ''): ", location.href.replace(/\/[^\/]*$/, ''));
 		oFileUploader = new jindo.FileUploader(jindo.$("uploadInputBox"),{
	 		// sUrl  : location.href.replace(/\/[^\/]*$/, '') + '/file_uploader.jsp',	//샘플 URL입니다.
	 		sUrl  : location.href.replace(/\/[^\/]*$/, '') + '/file_uploader.jsp',	//샘플 URL입니다.
 	        // sCallback : location.href.replace(/\/[^\/]*$/, '') + '/callback.html',	//업로드 이후에 iframe이 redirect될 콜백페이지의 주소
 			sCallback : '/admin/naver/se2/sample/photo_uploader/callback.html',	//업로드 이후에 iframe이 redirect될 콜백페이지의 주소
