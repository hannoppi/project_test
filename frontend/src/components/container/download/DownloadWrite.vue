<template>
  <section id="contents">
    <Hgr title="다운로드 게시판 등록" descript="이곳에서 다운로드 게시판 내용을 등록할 수 있습니다."></Hgr>
    ...
    <div class="board_gravity">
      <div class="inner_gravity">
        <button type="button" class="button_smarteditor" v-on:click="downloadWrite">등록</button>
        <router-link v-bind:to="{name: 'DownloadList'}" class="button_global">취소</router-link>
      </div><!-- // inner_gravity -->
    </div><!-- // board_gravity -->
  </section><!-- // contents -->
</template>
...
export default {
  ...
  data () {
    return { id: this.id, name: this.name, category: '다운로드', subject: '', content: '', htmlForEditor: '' }
  },
  mounted () {
  ...
  methods: {
    handleImageAdded: function (file, Editor, cursorLocation, resetUploader) {
      ...
      axios({
        url: '/api/admin/vuejs/download/write.action',
        method: 'POST',
        data: formData
      }).then((result) => {
