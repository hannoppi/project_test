<template>
  <section id="contents">
    <Hgr title="이미지 게시판 등록" descript="이곳에서 이미지 게시판 내용을 등록할 수 있습니다."></Hgr>

    <table class="table_global">
      <caption>이미지 게시판에 등록할 게시물의 카테고리, 제목, 내용을 입력할 수 있습니다.</caption>
    ...
    <div class="board_gravity">
      <div class="inner_gravity">
        <button type="button" class="button_smarteditor" v-on:click="galleryWrite">등록</button>
        <router-link v-bind:to="{name: 'GalleryList'}" class="button_global">취소</router-link>
      </div><!-- // inner_gravity -->
    </div><!-- // board_gravity -->
  </section><!-- // contents -->
</template>
...
export default {
  ...
  data () {
    return { id: this.id, name: this.name, category: '이미지', subject: '', content: '', htmlForEditor: '' }
  },
  mounted () {
