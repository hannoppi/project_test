<template>
  <section id="contents">
    <Hgr title="일반 게시판 목록" descript="이곳에서 일반 게시판 목록을 확인할 수 있습니다."></Hgr>
    ...
    <div class="paging" v-if="boardlist.length !== 0">
      ...
      <a href="#" class="next" v-else v-on:click="boardList">다음</a>
    </div><!-- // paging -->
    ...
    <div class="board_gravity">
      <div class="inner_gravity">
        <router-link v-bind:to="{name: 'BoardWrite'}" class="button_global">등록</router-link>
      </div><!-- // inner_gravity -->
    </div><!-- // board_gravity -->
  </section><!-- // contents -->
</template>
...
export default {
  ...
  created () {
    ...
    this.$store.commit(Constant.GNB, { menu: 'board' })
  },
  computed: mapState({
